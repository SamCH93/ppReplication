...
$\mathrm{Pr}(\alpha > 0.8 \,\vert\, \hat{\theta}_{r}) = \Sexpr{round(pbeta(q = 0.8, 1.5, 1, lower.tail = FALSE), 2)}$,
and for a threshold of 0.9 it is even lower
$\mathrm{Pr}(\alpha > 0.9 \,\vert\, \hat{\theta}_{r}) = \Sexpr{round(pbeta(q = 0.9, 1.5, 1, lower.tail = FALSE), 2)}$.
