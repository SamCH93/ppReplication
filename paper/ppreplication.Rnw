...
\date{July 28, 2022} %don't forget to hard-code date when submitting to arXiv!
...
\caption{Analysis of three replication studies of the ``Labels'' experiment from
  ...
  $\alpha \sim \Be(\Sexpr{round(x, 2)}, \Sexpr{round(y, 2)})$ prior
  ...
  incorporation of the original data. The dotted line represents the limiting
  posterior density of the power parameter $\alpha$ for perfectly agreeing
...
hypotheses about the parameters $\h{j}$ and $\h{k}$, say, by computing the Bayes
...
$\h{k}$, or as the relative predictive performance of $\h{j}$ and $\h{k}$ at
predicting the data \citep{Good1958, Jeffreys1961, Kass1995}.
...
It is of interest to understand the asymptotic behavior of the proposed Bayes
factors. For instance, we may wish to understand what happens when the standard
error of the replication study $\sigma_{r}$ becomes arbitrarily small (through
an increase in sample size). Assume that $\that_{r}$ is a consistent estimator
of its true underlying effect size $\theta_r$, so that as the standard error
$\sigma_r$ goes to zero, the estimate will converge in probability to the true
effect size $\theta_r$.
...
@
\caption{he heterogeneity $\tau^{2}$ and relative heterogeneity
...
heterogeneity under either hypothesis. Although this particular test relates to
the power parameter $\alpha$ in the power prior model, it is surprisingly
unrelated to testing the heterogeneity variance $\tau^2$ in the hierarchical
model.
...
%     (2\sigma^{2}_{o})/(2\tau^{2} + \sigma^{2}_o)^{2}
  ...
  \tau^2 \given \h{\text{c}}&\sim \mbox{GF}(y, 1, \sigma^2_o/2) &&&
...
gamma prior $\tau^2 \given \h{\text{d}} \sim \mbox{IG}(s, t)$ with shape $s$ and
scale $t$, the Bayes factor is given by
\begin{align*}
    \BF_{\text{dc}}(\that_r \given s, t)
    = \frac{\int \Nor(\that_r \given \that_o, \sigma^2_r + \sigma^2_o + 2\tau^2) \,
    \mbox{IG}(\tau^2 \given s, t) \, \text{d}\tau^2}{\Nor(\that_r \given \that_o,
    \sigma^2_r + \sigma^2_o)}
\end{align*}
with $\mbox{IG}(\cdot \given s, t)$ the density function of the inverse gamma
distribution. The limiting Bayes factor is therefore
\begin{align*}
    \lim_{\sigma_o, \sigma_r \downarrow 0} \BF_{\text{dc}}(\that_r \given s, t)
    = \frac{\Gamma(s + 1/2)\{t + (\theta_r - \theta_o)^2/4\}^{-(s + 1/2)}}{\delta(\theta_r - \theta_o) \, \sqrt{4\pi}},
\end{align*}
...
$\tau^2 \given \h{\text{d}} \sim \mbox{IG}(s,t)$ is consistent, but the original
...
\begin{align}
    \label{eq:igalpha}
    f(\alpha \given s, t)
    &= \frac{t^s}{\Gamma(s)} \, \frac{\alpha^{s - 1}}{(1 - \alpha)^{s + 1}} \,
    \left(\frac{2}{\sigma^2_o}\right)^{s} \,
    \exp\left\{-\frac{2 \,t\, \alpha}{\sigma^2_o(1 - \alpha)}\right\}.
\end{align}
...
available \citep{Gravestock2018}.
...
\url{https://doi.org/10.5281/zenodo.XXXXX}. We also provide an R package for
estimation and testing under the power prior framework at
