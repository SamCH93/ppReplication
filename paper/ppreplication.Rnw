...
%   distributions of effect size $\theta$ and power parameter $\alpha$ with 95\%
%   highest posterior density credible intervals for the latter. A power prior for
%   the effect size $\theta$ is constructed from the original effect estimate
...
%   $\Be(x = \Sexpr{round(x, 2)}, y = \Sexpr{round(y, 2)})$ prior distribution.
%   The dashed lines depict the posterior density for the effect size $\theta$
%   when the replication data are analyzed in isolation without incorporation of
%   the original data. The dotted line represents the limiting posterior density
%   of the power parameter $\alpha$ for perfectly agreeing original and
%   replication studies.}
...
testing hypotheses about these parameters. Let $H_{0}$ and $H_{1}$ denote two
competing hypotheses, each of them with an associated prior
$f(\theta, \alpha \given H_{i})$ and a resulting marginal likelihood obtained
...
the data $\that_{r}$ (second equality) \citep{Good1958, Jeffreys1961, Kass1995}.
...
% For example, for the effect size, we may want to test the hypotheses of the
% effect being absent ($\h{0} \colon \theta = 0$) versus the effect being present
% ($\h{1} \colon \theta \neq 0$). For the power parameter, we may want to contrast
% the hypotheses of the data sets being compatible
% ($\h{\text{c}} \colon \alpha = 1$) versus the data being different
% ($\h{\text{d}} \colon \alpha < 1$). A principled Bayesian approach is to compute
...
hypotheses $\h{0}$ is often unrealistic, it is usually a good approximation to
...
``different'' and the original data should be discounted to some extent
($\h{\text{d}} \colon \alpha < 1$).
...
absence of evidence for either absence or presence of an effect in the first
...
(column $\BF_{01}(\hat{\theta}_r \given \alpha = 1)$).
...
\begin{table}[!htb]
  ...
  \caption{Hypothesis tests for the replication studies of the ``\Sexpr{ex}''
    ...
    point prior $\alpha = 0$ with initial unit-information prior for $\theta$
    that has unit variance $\kappa^2 = \Sexpr{k^2}$) or partial discounting
    prior ($\alpha \sim \Be(1, y)$) under $\h{\text{d}}$.}
...
Bayes factor $\BF_{\text{dc}}(\hat{\theta}_r \given y = 2)$ in the right-most
column with the partial discounting prior $\alpha \sim \Be(1, y = 2)$ assigned
under the hypothesis $\h{\text{d}}$ indicates absence of evidence for either
hypothesis in the first and second replication, but strong evidence for
incompatibility $\h{\text{d}}$ in the third replication. The apparent
differences to the Bayes factor with the complete discounting prior (column
$\BF_{\text{dc}}(\hat{\theta}_r \given \kappa^2 =2)$) illustrate that in case of
no conflict (study 2) or not too much conflict (study 1) the test with the
partial discounting prior is less sensitive in inferring (in)compatibility, but
in case of substantial conflict (study 3) it is more sensitive. This analysis is
based on a prior with $y = 2$ corresponding to a linearly decreasing prior for
$\alpha$, Figure~\ref{fig:sensitivityPrior} shows the Bayes factor for other
values of $y$. We see that in the realistic range of $y = 1$ (uniform prior) to
$y = 100$ (almost all mass at $\alpha = 0$) the results for the first and third
replication hardly change, while the for the second replication the Bayes factor
shifts from anecdotal evidence to stronger evidence for compatibility.
...
ggplot(data = bfDF2, aes(x = y, y = bfdcRandom, color = trFormat)) +
    ...
    theme(panel.grid.minor = element_blank(),
          legend.position = "top")
@
\caption{Sensitivity analysis on the sensitivity of the Bayes factor
  $\BF_{\text{dc}}(\hat{\theta}_r \given y)$ with respect to the parameter
  $y$ of the partial discounting prior $\alpha \sim \Be(1, y)$ under the
  hypothesis of incompatibility $\h{\text{d}}\colon \alpha < 1$.}
\label{fig:sensitivityPrior}
\end{figure}

To conclude, our analysis suggests that only the second replication was
completely successful in the sense that it is both compatible with the original
study while also providing evidence for the presence of an effect. For the other
two replications the conclusions are more nuanced: In the first replication
there is no evidence for incompatibility with the original study but absence of
evidence for presence or absence of an effect. In the third replication there is
decisive evidence for an effect but also strong evidence for incompatibility
...
the hence the posterior probability of $\h{\text{c}}\colon \alpha = 1$ cannot
converge to one.
...
conclusive analysis in each case. For example, sample size calculations may be
performed based on posterior tail probabilities as in \citet{Pawel2022d}. Here,
we focus on sample size calculations based on the hypothesis tests described
previously. In the case of the replication Bayes factor under
...
result, standard Bayesian design \citep{Weiss1997, DeSantis2004,
...
\caption{Prior for the power parameter $\alpha$ implied by an inverse gamma
  prior $\tau^2 \given \h{\text{d}} \sim \mbox{IG}(q, r)$ in a hierarchical
...
$\alpha$ hardly changes from the prior \citep{Pawel2022c}. Similarly, Bayes
factors for testing the compatibility hypothesis $\h{\text{c}}\colon \alpha = 1$
are inconsistent.
...
more research is needed on the connection in more situations that depart from
...
Małgorzata Roos for helpful comments on a draft of the manuscript. We thank the
associate editor and the two anonymous reviewers for helpful comments and
