...
\begin{center}
  \begin{minipage}{13cm} {\small
      ...
      {\centering \textbf{Abstract} \\
        ...
        on heterogenetiy variances, and also how prior models can alternatively
        estimated through a hierarchical framework.
      } \\
...
## x <- 1
## y <- 1
## integrate(f = intFunAlpha, lower = 0, upper = 1, tr = tr, sr = sr, to = to,
##           so = so, x = x, y = y)
@
...
bfDF <- do.call("rbind", lapply(X = seq(1, length(tr)), FUN = function(i) {
    ...
    bf01 <- bfPPtheta(tr = tr[i], sr = sr[i], to = to, so = so, x = 1, y = 1)
