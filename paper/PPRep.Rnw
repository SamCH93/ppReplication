...
## compute posterior of effect size without using original data
...
\caption{Bayesian analysis of three replication studies from the replication
  ...
  The dashed lines indicate posterior density for the effect size when the
  replication data are analysed without incorporating the original data.
...
models is thus $\alpha \approx 1 - I$.
...
##     ## geom_abline(intercept = 1, slope = -1, lty = 2, alpha = 0.3) +
##     geom_line() +
##     labs(x = bquote(italic("I")^2), ## bquote(I^2 == frac(tau^2, tau^2 + sigma["o"]^2)),
##          y = bquote(alpha)) +
##     coord_fixed() +
##     theme_bw() +
##     theme(panel.grid.minor = element_blank())
...
## plot with I and I^2
plotDF$x <- plotDF$I2
plotDF$type <- "italic('I')^2"
plotDF2 <- plotDF
plotDF2$x <- sqrt(plotDF$x)
plotDF2$type <- "italic('I')"
plotDF3 <- rbind(plotDF, plotDF2)
plotDF3$type <- factor(plotDF3$type, levels = c("italic('I')^2", "italic('I')"))
ggplot(data = plotDF3, aes(x = x, y = alpha)) +
...
$ \tilde{\theta} = (\that_{o})/\{1 - (s \sigma^{2}_{o})/\kappa^{2}\} = \Sexpr{round(trueESmin, 2)}$,
